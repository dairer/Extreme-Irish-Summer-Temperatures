...
prep_rpareto_bts = function(marg_mod, bts_range, num_quantiles){
  ...
  for(file_name in bts_range){
    ...
    if(marg_mod == 'mod_0'){
      ...
      this_data <- readRDS(paste0("data/processed/bootstrap_data/bts_under_gpd_models/num_quantiles_30_bts_", file_name))%>%
        ...
        dplyr::select(-c(maxtp_1, maxtp_2)) %>%
      ...
      this_fit_mod = read_csv("output/gpd_model_fits/model_0_pars_corrected.csv",
      ...
      pred <- my_predict_0(this_fit_mod, this_data$scale_9)
      ...
      this_data <- readRDS(paste0("data/processed/bootstrap_data/bts_under_gpd_models/num_quantiles_30_bts_", file_name))%>%
        ...
        dplyr::select(-c(maxtp_0, maxtp_2)) %>%
      ...
      pred <- my_predict_1(this_fit_mod, this_data$scale_9, this_data$loess_temp_anom)
      ...
      this_data <- readRDS(paste0("data/processed/bootstrap_data/bts_under_gpd_models/num_quantiles_30_bts_", file_name))%>%
        ...
        dplyr::select(-c(maxtp_1, maxtp_0)) %>%
        left_join(covariates %>% dplyr::select(-c(scale_9, threshold_9)), by = c('year', 'Station'))%>%
        filter(maxtp>0)
  ...
  obs_data_standardised = this_data %>%
      ...
      group_map(~{
        ...
        .x$unif = res
        .x$frechet_marg = -1/log(.x$unif) # -- transform data to frechet margins, can equivalently be Pareto
        .x
...
# # ---- each of these runs many jobs locally, so be careful if you dont have many cores!!

# job::job({prep_rpareto_bts('mod_0', seq(1,100), 15)})
# job::job({prep_rpareto_bts('mod_0', seq(101,200), 15)})
# job::job({prep_rpareto_bts('mod_0', seq(201,300), 15)})
# job::job({prep_rpareto_bts('mod_0', seq(301,400), 15)})
# job::job({prep_rpareto_bts('mod_0', seq(401,500), 15)})
# job::job({prep_rpareto_bts('mod_0', seq(41,80), 15)})
# job::job({prep_rpareto_bts('mod_0', seq(81,100), 15)})
# job::job({prep_rpareto_bts('mod_0', seq(101,140), 15)})
# job::job({prep_rpareto_bts('mod_0', seq(141,180), 15)})
# job::job({prep_rpareto_bts('mod_0', seq(181,200), 15)})
# job::job({prep_rpareto_bts('mod_0', seq(201,240), 15)})
# job::job({prep_rpareto_bts('mod_0', seq(241,280), 15)})
# job::job({prep_rpareto_bts('mod_0', seq(281,300), 15)})


# job::job({prep_rpareto_bts('mod_1', seq(1,40), 15)})
...
# job::job({prep_rpareto_bts('mod_2', seq(1,40), 25)})
# job::job({prep_rpareto_bts('mod_2', seq(41,80), 25)})
# job::job({prep_rpareto_bts('mod_2', seq(81,100), 25)})
# job::job({prep_rpareto_bts('mod_2', seq(101,140), 25)})
# job::job({prep_rpareto_bts('mod_2', seq(141,180), 25)})
# job::job({prep_rpareto_bts('mod_2', seq(181,200), 25)})
# job::job({prep_rpareto_bts('mod_2', seq(201,240), 25)})
# job::job({prep_rpareto_bts('mod_2', seq(241,280), 25)})
# job::job({prep_rpareto_bts('mod_2', seq(281,300), 25)})
# job::job({prep_rpareto_bts('mod_2', seq(301,340), 25)})
# job::job({prep_rpareto_bts('mod_2', seq(341,380), 25)})
# job::job({prep_rpareto_bts('mod_2', seq(381,400), 25)})
# job::job({prep_rpareto_bts('mod_2', seq(401,440), 25)})
# job::job({prep_rpareto_bts('mod_2', seq(441,480), 25)})
# job::job({prep_rpareto_bts('mod_2', seq(481,500), 25)})
# job::job({prep_rpareto_bts('mod_2', seq(454,480), 25)})
# job::job({prep_rpareto_bts('mod_2', seq(492,500), 25)})
