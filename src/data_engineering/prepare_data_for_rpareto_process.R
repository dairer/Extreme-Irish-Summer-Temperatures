# --- this scripts processes data such that we can fit an r-pareto process
...
prep_rpareto_true = function(marg_mod){
  ...
  if(marg_mod == 'mod_0'){
    ...
    this_fit_mod = read_csv("output/gpd_model_fits/model_1_true.csv") %>%
      unlist() %>% as.numeric
    pred <<- my_predict_1(this_fit_mod, obs_data$scale_9, obs_data$loess_temp_anom)
    
  }else if(marg_mod == 'mod_2'){
    this_fit_mod = read_csv("output/gpd_model_fits/model_2_true.csv") %>%
      unlist() %>% as.numeric
    pred <<- my_predict_2b(this_fit_mod, obs_data$scale_9, obs_data$loess_temp_anom, obs_data$dist_sea)
  }
