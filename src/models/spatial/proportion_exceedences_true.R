...
if(file.exists(paste0("output/simulations/rescaled_simulations_on_obs_grid/true/",marg_mod,"_yr_",yr,"_min_temp_conditioned_on_", temp_conditioned_on))){
  ...
  # tibble(temp = seq(27, 36, by = 0.25), prop_ex_1942, prop_ex_2020) %>%
  #   write_csv(paste0("output/simulations/simulation_summary/prop_exceedance_model_true_",marg_mod,".csv"), append = T)
}
